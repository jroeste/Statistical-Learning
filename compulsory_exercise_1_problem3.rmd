...
* $\pi_k=Pr(Y=k)$ is the prior probability, which in this case gives the probability of being in class 1 or 2.$\mu_k$ is the mean for the two wine classes, and $f_k({\bf x})$ is the probability density function, given a class. $\Sigma$ is the covariate matrix, where the diagonal consists of the standard deviation elements, which are equal for all classes.

* We can estimate the prior probability $\pi_k$ for the wines by taking the fraction of obervations from class $k$: $\pi_k=\frac{n_k}{n}$, where $n$ is the total number of observations.

```{r, echo=FALSE}
library(plyr)
freq_train=count(train,"y") 
n0=freq_train[1,2]
n1=freq_train[2,2]
pi=c(n0/n,n1/n)
...
sigma_0=matrix(0,2,2)
sigma_1=matrix(0,2,2)
Sigma_train=matrix(0,2,2)
x_vec=c(0,0)
for (i in (1:n0)){
  
  x_vec=c(x_01[i],x_02[i])
  sigma_0=sigma_0+(x_vec-mu_0)%*%t(x_vec-mu_0)
  
}
sigma_0=sigma_0*1/(n0-1)

for (i in (1:n1)){
  
  x_vec=c(x_11[i],x_12[i])
  sigma_1=sigma_1+(x_vec-mu_1)%*%t(x_vec-mu_1)
}
sigma_1=sigma_1*1/(n1-1)
