...
To get an indication of which K-value is the best to choose, one has to compute the MSE-value for a test set of other data points than in the training set. For the test set the mean-squared errors has a minimum value for K around 3 to 5 and increases for higher and lower values. This indicates that choosing a K-value from 3 to 5 would give the best model fit. 
...
bp
```
We want to construct a 90% prediction interval for our prediction, and name the prediction for the response $\mathbf{\hat{Y}}=\mathbf{x_0^T}\boldsymbol{\beta}$ and constuct the expression for the prediction interval for $\mathbf{Y}$:
...
i) LineÃƒÂ¦r ok, sett inn utregning + plot.
ii) 
iii) Interpret beta 1 og 2?
iv)Formel fra boka + spm om boundary skal vÃƒÂ¦re lineÃƒÂ¦r.
