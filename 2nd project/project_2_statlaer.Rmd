...
$$
\boldsymbol{\hat Y} = -18.504-0.0065\text{weight}+0.8067\text{year}
$$
...
Q15: The plot shows how the MSE vary with the value of lambda. We want the MSE to be as small as possible, but we also want regularization to get a simpler model. Thus, two optimal $\lambda$-values are suggested by the plot, shown by a dotted line. The leftmost is that with lowest MSE value, and the other is 1se which correspond to the highest $\lambda$-value such that the MSE not more than one standard error away from the minimum.
