...
##c)
In this task, `modelA` is used to analyse the connection between $BMI$ and the response. 

* The estimated numerical value of $\hat\beta_{BMI}$ is found from the output of the provided code: $\hat\beta_{BMI}\approx 3.087e-04$.

* The estimated coefficients can be iterpreted as a measure on how much each covariate affects the predicted response, or the average effect on the response as a consequence of a unit change of the $BMI$-covariate. Considering $\hat\beta_{BMI}$ and comparing it to the other estimated coefficient, it seems like for `modelA`, the $BMI$ covariate has a significant impact on the response. This means that if you keep the other covariates constant and increase the $BMI$ covariate with one unit, the changes in the predicted response is $3.087e-04$.

* 99% confidence interval for  $\beta_{BMI}$ is the interval we are $99%$ confident to find $\beta_{BMI}$. It can be found using the standard error from the fitted model, $SE(\hat\beta_{BMI})^2=\frac{\sigma^2}{\sum_{i=1}^{n}}(x_i-\bar x)=2.955e-05$. The confidence interval is given by

$$
\hat\beta_{BMI}\pm t_{\alpha/2,n-2}\cdot SE(\hat\beta_{BMI})
$$
where the t-distribuition can be approximated by the normal distribution because $n$ is large. In this case, $\alpha=0.05$, so 
$$
...
[dette ble jo helt latterlig feil]
Therefore, on average we are quite confident that if one ignores all the other covariates, an increase of one $BMI$-unit, there will be an average decrease in the systolic blood pressure of between $-4299.2^2$ and $-2598.8^2$ units.
...
* Using $modelA$, one obtain the value $0.0867$ for his $1/sqrt(SYSBP)$ and $133$ for his $SYSBP$.
