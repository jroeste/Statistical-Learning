...
* The estimated numerical value of $\hat\beta_{BMI}$ is found from the output of the provided code: $\hat\beta_{BMI}\approx 3.087 \cdot 10^{-4}$.
...
* 99% confidence interval for  $\beta_{BMI}$ is the interval we are $99%$ confident to find $\beta_{BMI}$. It can be found using the standard error from the fitted model, $SE(\hat\beta_{BMI})^2=\frac{\sigma^2}{\sum_{i=1}^{n}}(x_i-\bar x)=2.955 \cdot 10^{-5}$. The confidence interval is given by
