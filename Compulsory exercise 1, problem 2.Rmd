...
The followig is a summary of this model, a "fitted values vs residuals"-plot, a Q-Q-plot and the Anderson-Darling normality test. 
...
* The estimated coefficients can be iterpreted as a measure on how much each covariate affects the response. Considering the rather low value of $\hat\beta_{BMI}$, it seems like for `modelA`, the $BMI$ covariate has a low impact on the response. This means that if you keep the other covariates constant and increase the $BMI$ covariate with one unit, one will most likely not observe any changes in the response.
